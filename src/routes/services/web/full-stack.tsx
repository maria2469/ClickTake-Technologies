        ...
        <div className="relative min-h-screen text-foreground overflow-x-hidden">
                ...
                <div className="max-w-6xl mx-auto">
                    <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.1 } } }}>
                        ...
                        <motion.h1 variants={fadeUp} className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[0.95] max-w-4xl">
                            ...
                            <span className="bg-gradient-to-r from-brand-cyan via-brand-cyan to-emerald-400 bg-clip-text text-transparent">
                                built to last.
                            </span>
                        </motion.h1>
                        ...
                        <motion.div variants={fadeUp} custom={2} className="mt-8 flex flex-wrap gap-3">
                            <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-cyan to-brand-cyan px-7 py-3.5 font-semibold text-white shadow-lg hover:scale-105 transition-transform">
                ...
                <div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-2 md:grid-cols-4 gap-8">
                    {results.map((r, i) => (
                        <motion.div key={r.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
                            <div className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-brand-cyan to-brand-cyan bg-clip-text text-transparent">{r.metric}</div>
            ...
            <section className="relative z-10 py-24 px-4">
                <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="rounded-2xl border border-brand-cyan/20 bg-gradient-to-br from-brand-cyan/5 to-brand-cyan/5 p-8 backdrop-blur">
                        <div className="text-xs font-semibold uppercase tracking-widest text-brand-cyan mb-4">Our Standards</div>
                        ...
                        {deliverables.map((d, i) => {
                            ...
                            return (
                                <motion.div key={d.label} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                                    ...
                                    <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-brand-cyan/20 to-brand-cyan/20 flex items-center justify-center">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {process.map((p, i) => (
                            <motion.div key={p.step} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                                ...
                                <div className="text-5xl font-black text-transparent bg-gradient-to-br from-brand-cyan/30 to-brand-cyan/30 bg-clip-text mb-4 select-none group-hover:from-brand-cyan/60 group-hover:to-brand-cyan/60 transition-all">{p.step}</div>
                        ...
                        {differentiators.map((d, i) => {
                            ...
                            return (
                                <motion.div key={d.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                                    ...
                                    <div className="h-11 w-11 shrink-0 rounded-xl bg-gradient-to-br from-brand-cyan/10 to-brand-cyan/10 border border-border flex items-center justify-center group-hover:border-brand-cyan/30 transition-colors">
                                        <Icon className="h-5 w-5 text-brand-cyan" />
                                    </div>
                                    <div>
            ...
            <CtaSection>
                <div className="max-w-3xl mx-auto text-center">
                    <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                        <div className="text-xs font-semibold uppercase tracking-widest text-brand-cyan mb-4">Ready to build?</div>
                        <h2 className="text-4xl sm:text-5xl font-bold tracking-tight mb-5">
                            An idea isn't a product.<br />
                            <span className="bg-gradient-to-r from-brand-cyan to-brand-cyan bg-clip-text text-transparent">Let's build yours properly.</span>
                        </h2>
                        ...
                        <div className="flex flex-wrap gap-3 justify-center">
                            <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-cyan to-brand-cyan px-8 py-4 font-semibold text-white shadow-lg hover:scale-105 transition-transform text-base">
