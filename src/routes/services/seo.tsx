    ...
    <div className="relative min-h-screen text-foreground overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto">
          <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.1 } } }}>
            ...
            <motion.h1 variants={fadeUp} className="text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-[0.95] max-w-4xl">
              Get found by people{" "}
              <span className="bg-gradient-to-r from-brand-cyan via-emerald-400 to-teal-500 bg-clip-text text-transparent">
                ready to buy.
              </span>
            </motion.h1>
            ...
            <motion.div variants={fadeUp} custom={2} className="mt-8 flex flex-wrap gap-3">
              <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-cyan to-teal-500 px-7 py-3.5 font-semibold text-white shadow-lg hover:scale-105 transition-transform">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-2 md:grid-cols-4 gap-8">
          {results.map((r, i) => (
            <motion.div key={r.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              <div className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-brand-cyan to-emerald-400 bg-clip-text text-transparent">{r.metric}</div>
      ...
      <section className="relative z-10 py-24 px-4">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="rounded-2xl border border-brand-cyan/20 bg-gradient-to-br from-brand-cyan/5 to-emerald-500/5 p-8 backdrop-blur">
            ...
            {deliverables.map((d, i) => {
              ...
              return (
                <motion.div key={d.label} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                  ...
                  <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-brand-cyan/20 to-emerald-500/20 flex items-center justify-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {process.map((p, i) => (
              <motion.div key={p.step} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                <div className="text-5xl font-black text-transparent bg-gradient-to-br from-brand-cyan/30 to-emerald-500/30 bg-clip-text mb-4 select-none group-hover:from-brand-cyan/60 group-hover:to-emerald-500/60 transition-all">{p.step}</div>
            ...
            {differentiators.map((d, i) => {
              ...
              return (
                <motion.div key={d.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                  ...
                  <div className="h-11 w-11 shrink-0 rounded-xl bg-gradient-to-br from-brand-cyan/10 to-emerald-500/10 border border-border flex items-center justify-center group-hover:border-brand-cyan/30 transition-colors">
                    <Icon className="h-5 w-5 text-brand-cyan" />
                  </div>
                  <div>
            ...
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-brand-cyan/20 bg-brand-cyan/5 p-6">
                  <div className="text-sm font-semibold text-brand-cyan mb-1">Monthly Retainer</div>
      ...
      <CtaSection>
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="text-xs font-semibold uppercase tracking-widest text-brand-cyan mb-4">Ready to rank?</div>
            <h2 className="text-4xl sm:text-5xl font-bold tracking-tight mb-5">
              Your competitors are already investing.<br />
              <span className="bg-gradient-to-r from-brand-cyan to-emerald-400 bg-clip-text text-transparent">Don't let them compound.</span>
            </h2>
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              <a href="#contact" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-cyan to-emerald-500 px-8 py-4 font-semibold text-white shadow-lg hover:scale-105 transition-transform text-base">
